...
description: Deconstruct complex features into decoupled, parallel task specifications for external agents (e.g. OpenCode, subagents), track execution with a master checklist markdown, enforce mandatory walkthrough artifact generation, and perform rigorous review and quality assurance. Use when delegating tasks to external coding agents, splitting work into parallel tracks (BE/FE/Docs), or reviewing multi-agent outputs.
---
...
1. **Native Compatibility with `executing-plans`**
   - External agents use `executing-plans` to implement tasks. Track task specs are therefore formatted natively as `writing-plans` implementation plans with granular, checkbox-driven TDD steps (`- [ ] Step 1: ...`).
...
1. **Goal & Architecture Constraints** (zero N+1 queries, type-safety, backward compatibility).
...
3. MANDATORY: Generate local/agents/brainstorming/plans/<track>_walkthrough.md
   with terminal verification logs before finishing.
```
